...
def get_public_config():
    ...
    # For data segment
    parser.add_argument('--order', type=int, default=0) # 0-位移；1-速递；2-加速度. 考虑总数为1+2*order
    parser.add_argument('--new_node_ratio', type=float, default=0.1)
    parser.add_argument('--num_val', type=int, default=1)
    parser.add_argument('--epsilon', type=float, default=0.01) # 衡量点之间的相似度 for Hamming Metric
    parser.add_argument('--c', type=int, default=1) # Bourgain定理中Anchorset重复采样次数
    ...
    parser.add_argument('--beta0', type=int, default=2) # L = L_ob + gammaL_un'
    return parser
